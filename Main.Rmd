...
>We will proceed to train the model using random forest and a 3-fold CV. This is done because of the discrete and categorical nature of the classe column.

```{r training}
# instruct train to use 3-fold CV to select optimal tuning parameters
fitctrl <- trainControl(method="cv", number=3, verboseIter=F)
mod0 <- train(classe ~ ., data=train, method="rf", trControl = fitctrl)
...
>There seems to be a good model fit, that might predict well on small data sets. Nothing can be said about bigger sets, unfortunately.
